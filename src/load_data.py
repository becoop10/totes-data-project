...
def get_secret():
    '''Retrieves data-warehouse db connection details from
        aws secrets manager'''
...
def read_csv(s3, path, bucket):
    '''Reads the contents of a csv object in an s3 bucket
...
def read_parquets(s3, path, bucket):
    '''Reads the contents of a parquet object in an s3 bucket
...
def query_builder(r, filename, invocations):
    ...
    if 'dim' in filename:
        var_in = (tuple(values), )
        query = f'''INSERT INTO {filename} ({key_string})
                    VALUES %s
                    ON CONFLICT ({id_columns[filename]})
                    DO UPDATE SET {full_update_string};'''
    else:
        ...
        query = f'DO $$ declare comparison INT:={id_v}; \
                BEGIN \
                IF comparison IN (select {id_columns[filename]} \
                FROM {filename}) THEN \
                UPDATE {filename} SET {full_fact_update_string} \
                WHERE {id_columns[filename]}={id_v}; \
                ELSE \
                        ...
                        if file == f'{f}':
                            ...
                            if response == 0:
                                if filename == 'fact_sales_order':
                                    cur.execute(
                                        f"ALTER TABLE {filename} \
                                            ALTER COLUMN sales_record_id \
                                                RESTART WITH 1;")
                                if filename == 'fact_purchase_order':
                                    cur.execute(
                                        f"ALTER TABLE {filename} \
                                            ALTER COLUMN purchase_record_id \
                                                RESTART WITH 1;")
                                if filename == 'fact_payment':
                                    cur.execute(
                                        f"ALTER TABLE {filename} \
                                            ALTER COLUMN payment_record_id \
                                                RESTART WITH 1;")
                                ...
                                if filename == "dim_transaction":
                                    sorted_data['sales_order_id'] = sorted_data['sales_order_id'].astype(
                                        'Int64')
                                    sorted_data['purchase_order_id'] = sorted_data['purchase_order_id'].astype(
                                        'Int64')
                                if 'fact' in filename:
                                    headers.pop(0)
                                ...
                                if 'fact' in filename:
                                    ...
                                    query = f'COPY {filename} ({headers_string}) \
                                        FROM STDIN DELIMITER \',\' CSV HEADER'
                                    cur.copy_expert(
                                        query, open(csv_file_name, "r"))
                                else:
                                    query = f'COPY {filename} \
                                        FROM STDIN DELIMITER \',\' CSV HEADER'
                                    cur.copy_expert(
                                        query, open(csv_file_name, "r"))
